...
class Runner:
    ...
    def gpu_runner_core(self, explist, lock):
        while True:
            ...
            # Build exactly the same log-directory name as below.
            if (
                self.running_params['treat_graph_based_features_like_one_hot']
                and initial_feature_type == 'graph_based'
            ):
                feature_type_name = 'graph_based+'
            else:
                feature_type_name = initial_feature_type
            
            log_dir = os.path.join(
                self.running_params['log_dir'],
                dataset_name + ('_log' if self.running_params['log_ibd'] else ''),
                f'{model}_{feature_type_name}_split_{split_id}',
            )
            
            results_path = os.path.join(log_dir, 'results.json')
            
            if os.path.isfile(results_path):
                print(f'Skipping completed run: {results_path}')
                continue
